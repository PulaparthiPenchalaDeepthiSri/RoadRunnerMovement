...
let allTripsData = {};
const existingCards = {}; // Track existing card elements
...
/* ===== OPTIMIZED FIREBASE LISTENER - NO BLINKING! ===== */
function listenFirebase() {
  onValue(ref(db), async snap => {
    ...
    // Track existing trip IDs
    const currentTripIds = new Set(trips.map(([id]) => id));
    ...
    for (const [id, t] of trips) {
      ...
      if (!existingCards[id]) {
        ...
        // Update existing card content without recreating
...
/* ===== CREATE TRIP CARD (Only called once per trip) ===== */
function createTripCard(id, t, loc) {
  ...
  card.innerHTML = `
    <div class="trip-card-header">
      ...
      <div class="trip-status-badge"></div>
    </div>
    ...
    <div class="trip-info-grid">
      <div class="trip-info-item">
        ...
        <div class="trip-info-value trip-driver">${t.driverName}</div>
      </div>
      
      <div class="trip-info-item">
        <div class="trip-info-label">Coordinator</div>
        <div class="trip-info-value trip-coordinator">${t.coordinatorName || "N/A"}</div>
      </div>
      
      <div class="trip-info-item">
        <div class="trip-info-label">Employee Code</div>
        <div class="trip-info-value trip-code">${t.coordinatorCode || "N/A"}</div>
      </div>
      
      <div class="trip-info-item">
        <div class="trip-info-label">At Disposal</div>
        <div class="trip-info-value trip-disposal">${t.atDisposal ? "Yes" : "No"}</div>
      </div>
    </div>
...
/* ===== UPDATE TRIP CARD (Updates content without recreating) ===== */
function updateTripCard(card, id, t, loc) {
  // Update status badge
  const statusBadge = card.querySelector(".trip-status-badge");
  if (t.status === "active") {
    statusBadge.innerHTML = '<div class="trip-badge"><div class="badge-pulse"></div>ACTIVE</div>';
  } else {
    statusBadge.innerHTML = '<div class="trip-badge ended">ENDED</div>';
  }

  // Update dynamic fields if they changed
  const driverEl = card.querySelector(".trip-driver");
  if (driverEl && driverEl.textContent !== t.driverName) {
    driverEl.textContent = t.driverName;
  }

  const coordinatorEl = card.querySelector(".trip-coordinator");
  if (coordinatorEl && coordinatorEl.textContent !== (t.coordinatorName || "N/A")) {
    coordinatorEl.textContent = t.coordinatorName || "N/A";
  }
}
